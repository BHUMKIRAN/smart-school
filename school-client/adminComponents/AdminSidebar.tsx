...
export default function AdminSidebar({
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-full bg-slate-900/50 backdrop-blur-xl border-r border-amber-500/20 transition-all duration-300 z-50 ${
    ...
    >
      ...
      <nav className="p-4 space-y-2">
        {navItems.map((item) => (
          ...
          >
            {item.icon}
            <span className={`nav-text font-medium text-sm ${!sidebarOpen && 'hidden'}`}>
              {item.label}
            </span>
          </button>
        ))}
