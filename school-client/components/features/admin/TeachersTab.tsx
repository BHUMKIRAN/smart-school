    ...
    <div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        {stats.map((stat, index) => (
          <div key={index} className="bg-white backdrop-blur-sm border border-indigo-500/20 rounded-xl p-6 hover:border-primary-500/40 transition-all">
            <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${stat.gradient} flex items-center justify-center mb-4`}>
              {stat.icon}
            </div>
            <h3 className="text-3xl font-bold text-gray-700 mb-1">{stat.value}</h3>
            <p className="text-sm text-gray-700">{stat.label}</p>
            <p className="text-xs text-indigo-400 mt-2">{stat.change}</p>
          </div>
        ))}
      </div>

      <div className="bg-white backdrop-blur-sm border border-primary-500/20 rounded-xl overflow-hidden">
        <div className="overflow-auto">
          <table className="w-full">
            <thead className="bg-white border-b border-primary-500/20">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Teacher</th>
            ...
            <tbody className="divide-y divide-slate-700/50">
              {teachers.map((teacher, index) => (
                <tr key={index} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{teacher.email}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{teacher.students}</td>
