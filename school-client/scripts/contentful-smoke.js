...
const run = async () => {
  ...
  console.log("hero items:", hero.items.length);
  ...
  console.log("about items:", about.items.length);
  ...
  console.log("committee items:", committee.items.length);

  const teachers = await client.getEntries({
    content_type: "teachers",
    include: 5,
    limit: 1,
  });
  console.log("teachers items:", teachers.items.length);
};
